...
if "XMLY_SPEED_COOKIE" in os.environ:
    ...
    if "BARK_MACHINE_CODE" in env_dist:
        bark_machine_code = os.environ["BARK_MACHINE_CODE"]
    if "TG_BOT_TOKEN" in env_dist:
        TG_BOT_TOKEN = os.environ["TG_BOT_TOKEN"]
    if "TG_USER_ID" in env_dist:
    ...
    if "XMLY_ACCUMULATE_TIME" in os.environ and os.environ["XMLY_ACCUMULATE_TIME"] == 'True':
...
def main():
    ...
    if bj_dt.hour % 18 == 0 and bj_dt.hour / 18 == 1 and bj_dt.minute <= 30:
        send_data = {"chat_id":TG_USER_ID, "text":bark_content, "disable_web_page_preview":"true"}
        res = requests.post(url='https://api.telegram.org/bot%s/sendMessage' % (TG_BOT_TOKEN),data=send_data)
        print(res.text)
